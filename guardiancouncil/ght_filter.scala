...
class GHT_FILTER_IO (params: GHT_FILTER_Params) extends Bundle {
  ...
  val csr_rw_wdata                              = Input(UInt(params.xlen.W))
}
...
class GHT_FILTER (val params: GHT_FILTER_Params) extends Module with HasGHT_FILTER_IO
{
  ...
  val csr_rw_wdata_reg                          = RegInit(0.U(params.xlen.W))
  ...
  pc_reg                                       := pc
  csr_rw_wdata_reg                             := io.csr_rw_wdata
  ...
  io.packet_out                                := MuxCase(0.U, 
                                                    Array((dp_sel === 0.U)  -> 0.U,
                                                          ...
                                                          (dp_sel === 5.U)  -> Cat(pc_reg, inst_reg, csr_rw_wdata_reg(31,0), dp_1_reg(31,0))
                                                          )
                                                          ) 
}